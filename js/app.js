...
// showDetails function to call as a handler in details button
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p>Total ${product.rating.count} people rated this product.</p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button id="details-btn" onclick ="callDetails(${product.id})" class="btn btn-danger">Details</button></div>
      `;
...
// gets input value and returns a float value
const getInputValue = (id) => {
  const element = document.getElementById(id).innerText;
  const converted = parseFloat(element);
  // const convertedTwoDecimal = converted.toFixed(2);
  return converted;
};
